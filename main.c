...
double calc_circle_hit_ratio(int limit)
{
	int total_inside = 0;
	double x;
	double y;
...
void calc_circle_hit_ratio_mean(int how_many_throws, int how_many_times)
{
	...
	double mean = sum / (double)how_many_times;
	double proximicity = PIover4 - mean;
	printf("%i times%*s-> Mean = %f | Proximicity = %*s%f\n", how_many_times, 8 - int_len(how_many_times), "", mean,
	       proximicity > 0 ? 1 : 0, "", proximicity);
}
